...
class EmpleadoRepositoryMySqlIT {
    ...
    @BeforeAll
    static void checkMysqlAvailable() {
        assumeTrue(portOpen("localhost", 3306, 500), () -> "[MySQL IT] Saltando tests: puerto 3306 no disponible. ¿Está levantado docker compose?");
    }

    @BeforeEach
    void cleanDb() {
        // En caso de tener datos previos en la base
        empleadoRepository.deleteAll();
        departamentoRepository.deleteAll();
    }

    @Test
    @DisplayName("MySQL: CRUD/consultas básicas de EmpleadoRepository")
    void mysqlCrudAndQueries() {
        ...
        e1.setEmail("ana-mysql@example.com");
        ...
        e2.setNombre("Bruno");
        e2.setApellido("Diaz");
        e2.setEmail("bruno-mysql@example.com");
        ...
        e3.setEmail("carla-mysql@example.com");
        ...
        Optional<Empleado> byEmail = empleadoRepository.findByEmail("bruno-mysql@example.com");

        // then
        assertEquals(2, itEmployees.size(), "IT debe tener 2 empleados (MySQL)");
        assertEquals(1, salaryRange.size(), "El rango debe incluir solo 1 empleado (MySQL)");
        assertTrue(byEmail.isPresent(), "Debe encontrar empleado por email (MySQL)");
